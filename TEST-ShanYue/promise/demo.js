const CustomPromise = class {
		...
		try {
			const resolve = (result) => {
				if (this.status === CustomPromise.STATUS_MAP.Pending) {
					// Promise内部状态具有凝固效果，一但确定了就不再发生变化
					if (
						result !== null &&
	...
	then(onfulfilled, onrejected) {
		...
		if (this.status === CustomPromise.STATUS_MAP.Fulfilled) {
			// Promise对象在状态凝固之后仍然是可以调用then方法的
		...
		if (typeof onfulfilled === "function") {
			setTimeout(() => {
				...
				if (x === this.promises[index]) {
					this.reject[index](
						new TypeError(
							"[onFulfilled] return the same value with [then] function"
						)
					);
				}
		...
		if (typeof onrejected === "function") {
			setTimeout(() => {
				...
				if (x === this.promises[index]) {
					this.reject[index](
						new TypeError(
							"[onrejected] return the same value with [then] function"
						)
					);
				}
				this.resolutionProcedure(
